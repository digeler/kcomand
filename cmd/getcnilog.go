...
func getcnilogs(vmname string, rgname string, storagename string) {
	buf := bytes.Buffer{}
...
// getcnilogCmd represents the getcnilog command
var getcnilogCmd = &cobra.Command{
	Use:   "getcnilog",
	Short: "This will collect al CNI related logs",
	Long: `will collect :  /etc/cni/net.d/10-azure.conflist,azure-vnet-ipam.log,azure-vnet.log
	`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		getcnilogs(vmname, rgname, *n)

	},
}

func init() {
	rootCmd.AddCommand(getcnilogCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// getcnilogCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// getcnilogCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
